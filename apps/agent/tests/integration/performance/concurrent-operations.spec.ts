    ...
    it("should handle concurrent large file uploads", async function () {
      ...
      const uploadPromises = Array.from({ length: 5 }, (_, i) =>
        request(testServer.app)
          .post("/blob")
          .set("Authorization", `Bearer ${accessToken}`)
          .field("filename", `large-concurrent-${i}.bin`)
          .attach("file", largeFileData, `large-concurrent-${i}.bin`),
      );
      ...
      const totalTime = endTime - startTime;
      const avgTimePerMB =
        totalTime / (5 * (config.maxFileSize / (1024 * 1024)));

      console.log(
        `5 concurrent ${config.maxFileSize / 1024}KB uploads completed in ${totalTime}ms (${avgTimePerMB.toFixed(2)}ms per MB)`,
      );

      // Performance assertion
      expect(avgTimePerMB).to.be.lessThan(10000, "Large file uploads too slow");
    });
  });
